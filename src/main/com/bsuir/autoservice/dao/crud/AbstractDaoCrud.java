...
public abstract class AbstractDaoCrud<PrimaryKey, Entity extends Bean> implements IDaoCrud<PrimaryKey, Entity> {
    ...
    public boolean update(Entity entity, Map<String, String> conditionValues) throws DaoException {
        try {
            ...
            LinkedHashMap<String, String> map = entity.getFieldValuesStrings();
            List<String> s = entity.getAutoGeneratedFields();
            LinkedHashMap<String, String> as = new LinkedHashMap<>();
            for(Map.Entry<String, String> entry : map.entrySet()) {
                if(!s.contains(entry.getKey())){
                    as.put(entry.getKey(), entry.getValue());
                }
            }

            PreparedStatement ps = connection.prepareStatement(
                    sql.getUpdateQuery(getFullTableName(), conditionValues, as)
            );
    ...
    public boolean delete(Entity entity) throws DaoException {
        try {
            ...
            PreparedStatement ps = connection.prepareStatement(
                    sql.getDeleteQuery(getFullTableName(), as)
            );
            return ps.execute();
        }catch (Exception e){
            throw new DaoException(e);
